...
contract BatchTransfer {
    ...
    function emergencyWithdraw(address token, uint256 amount) external {
        if (msg.sender != _getOwner()) revert();
    ...
    function _getOwner() internal view returns (address) {
        // Simple ownership: deployer is owner
        // For production, consider using Ownable pattern
        return address(0); // Placeholder - implement proper ownership if needed
    }
